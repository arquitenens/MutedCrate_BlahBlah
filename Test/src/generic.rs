...
impl<T: Eq + Hash + Debug> Drop for Muted<T>{
    fn drop(&mut self) {
        let mut to_drop: Vec<(*mut Vec<Data<T>>, usize)> = Vec::new();
...
impl<T: Hash + Eq + Debug> Muted<T>{
    ...
    pub fn drop_vec(&mut self, index: Option<usize>, maybe_ptr: Option<*mut Vec<Data<T>>>) -> Option<()>{
    ...
    fn insert_vec_inner(&mut self, index: usize, vec: Vec<T>, skip_calibration: bool) -> Option<()>{
        ...
        if self.data[index] == Data::Rp(None) {
            self.r_hold.insert(Data::Rp(Some(ptr)), Some((other, index, len)));
            self.data[index] = Data::Rp(Some(ptr));
        }else {
            {
        ...
        return match &mut self.data[rough_index] {
            ...
            Data::Rp(p) => {
                if let Some(pointer) = p{
                    unsafe {
                        let vec: &mut Vec<Data<T>> = &mut **pointer;
